...
function createRoom({ code, name, password, ownerToken, accent, liveMode, maxMembers, omegle, walkieMode }) {
  const room = {
    code,
    name,
    ...
    liveMode: !!liveMode,
    maxMembers: Number.isFinite(maxMembers) && maxMembers > 0 ? Math.min(200, Math.floor(maxMembers)) : 0,
    omegle: !!omegle,
    walkieMode: !!walkieMode,
  };
